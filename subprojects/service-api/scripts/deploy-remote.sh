...
if [ "$PROD" == true ]; then
        PROJECT_FILES=$(echo "bin cli data config network scripts settings src tsconfig.json package.deploy.json .npmrc.server")
    else
        PROJECT_FILES=$(echo "bin cli data config network scripts settings src tsconfig.json package.deploy.json .npmrc.local")
fi
